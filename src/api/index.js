...
export const getFavoriteProducts = (username, sortBy, sortOrder, page) => API.get(`/products/${username}/favorites` + (sortBy || page ? '?' : '')  + (sortBy ? `&sortBy=${sortBy}&order=${sortOrder}` : '') + (page ? `page=${page}` : ''));
