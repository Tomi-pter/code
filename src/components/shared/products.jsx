...
export const Products = ({
  ...
  var startCount = 1;
  if (pageNumber > 1) startCount = (pageNumber - 1) * 12 + 1;
  if (pageNumber === totalPageCount) totalInPage = totalProduct;
  ...
  const handleSortCheck = (e) => {
    ...
    if (category === "Favorites") {
      dispatch(
        getFavoriteProducts(user?.username, null, filter, order, null, value)
      );
    } else {
      if (page === "search") {
  ...
  const handlePageClick = (data) => {
    ...
    if ((category !== "" && page === "shop") || page === "search") {
      if (category === "Favorites") {
        ...
        dispatch(
          getProducts(name, category, filter, order, increment, sortStock)
        );
      }
    }
  };
  ...
  useEffect(() => {
    ...
    if (search === "" && isLoading) {
      changeTimer = setTimeout(() => {
        dispatch(
          getFavoriteProducts(user?.username, null, filter, order, 1, sortStock)
        );
      }, 1000);
    }
  ...
  useEffect(() => {
    setIsLoading(true);
    if (page === "search")
      dispatch(getProducts(name, null, filter, order, 1, sortStock));
  }, [name]);
    ...
    <div className="products-container">
        ...
        {page === "search" ? (
          ...
          <p className="total-products">
            {isLoading || !totalProduct
              ...
              : totalProduct === 0
              ? ""
              : `Showing ${startCount} - ${totalInPage} of ${totalProduct} products`}
          </p>
        )}
            ...
            <div className="dropdown">
              ...
              <div className="dropdown-menu" aria-labelledby="sortByDropdown">
                ...
                <span
                  className="dropdown-item"
                  onClick={() => {
                    setSorting("price", "DESC", "$ High - Low");
                  }}
                >
                  $ High - Low
                </span>
                ...
                <div className="flex align-items-center dropdown-item">
                  <input
                    ...
                    id="sortStock"
                    name="sortStock"
                    defaultChecked={sortStock}
                    onChange={handleSortCheck}
                  />
                  <label className="mb-0" for="sortStock">
                    {" "}
                    Stock
                  </label>
                </div>
      ...
      <div className={"products" + (view === "list" ? " list-view" : "")}>
        <div className="list-header">
          ...
          <p
            className={"buy" + (!user ? " buy-offline" : " buy-online")}
            style={!user ? { minWidth: "90px" } : { minWidth: "145px" }}
          >
            Buy
          </p>
      ...
      <div className="pagination-products">
        <ReactPaginate
          ...
          containerClassName={
            totalProduct === 0 || isLoading ? "pagination empty" : "pagination"
          }
